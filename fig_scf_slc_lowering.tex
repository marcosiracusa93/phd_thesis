   ...
   (*\hlnone{ }*)slc.callback{
    ...
    (*\hlnone{ }*)f32 val=slc.to_val(vals);
     ...
     (*\hlcompute{out[bv,ev]=acc+val}*); }}}}}
            \end{lstlisting}
            \caption{SLC IR}
            \label{fig:lowering-slc}
        \end{subfigure}
    \end{subfigure}
    %\vspace{-1em}
    \caption[Lowering from SCF to Ember SLC IR.]{Lowering from SCF to our SLC IR. Ember converts SCF \hlloop{for-loops} into SLC for-loops and SCF \hlmem{memory accesses} into SLC streams. Ember wraps \hlcompute{computation} into SLC callbacks that can access SLC streams with stream-to-value operations.}
    \label{fig:lowering}
\end{figure}