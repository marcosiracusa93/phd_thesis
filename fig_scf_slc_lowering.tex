...
void eb(idxs: mref<? x idx>,
 ...
 (*\hlloop{slc.for(str b from 0 to n\_batches)}*){
  ...
  (*\hlloop{slc.for(str p from beg to end)}*){
   ...
   (*\hlloop{slc.for(str e from 0 to emb\_len)}*){
    (*\hlmem{str val=slc.mem\_str(vals[i,e])}*);
   (*\hlnone{ }*)slc.callback{
    (*\hlnone{ }*)idx bv=slc.to_val(beg);
    (*\hlnone{ }*)idx ev=slc.to_val(end);
    (*\hlnone{ }*)f32 vv=slc.to_val(val);
     (*\hlcompute{f32 acc=out[bv,ev]}*);
     (*\hlcompute{out[bv,ev]=acc+vv}*); }}}}}
            \end{lstlisting}
            \caption{SLC IR}
