...
Vector loads are more efficient when aligned to cache lines~\cite{eichenberger2004pldi,fireman2007algorithms}. However, as shown in \Cref{fig:opt-impact-buf}, the access unit pushes both scalar and vector operands into the data queue. Hence, the vector loads that the execute unit performs from the queue are misaligned by scalar operands. Ember implements two optimizations to align queue accesses according to code characteristics.
