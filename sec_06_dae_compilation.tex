...
\end{itemize}
Concretely, we show these streams in the lookup code for the EB example in \Cref{fig:dlc-lookup-code}. Lines 1--12 in \Cref{fig:dlc-lookup-code} represent lines 5--9 in \Cref{fig:eb-imperative-code}, which are compiled by Ember.

Compute code consists of callbacks that the access unit triggers while traversing and loading tensor operands. In \Cref{fig:eb-imperative-code}, we can wrap the compute operation in line 10 into a callback that the access unit can trigger at each iteration of its parent loop. To trigger these callbacks, the access unit enqueues the corresponding token (the inner-loop iteration token ($e_i$)) in the control queue and its operands (result coordinates $b$ and $e$ and value $v$) in the data queue. Lines 14--17 in \Cref{fig:dlc-lookup-code} and lines 2--4 in \Cref{fig:dlc-compute-code} show how these tokens and data are enqueued and dequeued from the queues, and the exact queue content is shown in \Cref{fig:dlc-queue-content}. \Cref{fig:opt-impact-aligned} shows a more complex example with multiple callbacks. Formally, we can program the access unit to marshal control tokens and operands at three traversal locations---at each iteration, beginning, and end of a loop---through the following operations:
\begin{itemize}
...
\Cref{fig:slc-ir} shows the SLC grammar, and \Cref{fig:slc-lowering-output} shows the SLC IR of the EB function example. The main goal of the SLC IR is to abstract the queue (de)serialization logic in the DLC IR, enabling global DAE optimizations.
...
After performing global optimizations, the SLC IR is lowered to the low-level DLC IR. The DLC IR is generated by traversing the SLC IR from the outer to the inner SLC for-loop. SLC for-loops and streams are lowered to DLC traversal operators and streams. Callbacks, instead, are moved into a while-loop in the compute code (\Cref{fig:dae-abstraction}). Multiple callbacks are chained into an if-then-else construct that dispatches on the token IDs from the control queue (e.g. \Cref{fig:opt-impact-aligned}). Ember generates push instructions for control tokens based on the location of the callback in the SLC IR and queueing instructions for operands according to the SLC stream-to-value operations.
...
Vector loads are more efficient when aligned to cache lines~\cite{eichenberger2004pldi,fireman2007algorithms}. However, as shown in \Cref{fig:opt-impact-buffered}, the access unit pushes both scalar and vector operands into the data queue. Hence, the vector loads that the execute unit performs from the queue are misaligned by scalar operands. Ember implements two optimizations to align queue accesses according to code characteristics.

For simpler functions like the EB in \Cref{fig:opt-buffered}, where all segment IDs are just loop induction variables, Ember stores a reference of these indices in the core and increments them after the loop. This increment is triggered by a segment-end token, $s_e$ in \Cref{fig:opt-impact-aligned}.

As shown in \Cref{fig:opt-aligned}, Ember implements this optimization by examining iteration callbacks for stream-to-value operations that only read the induction variable of their own SLC(V) loop (e.g. line 13 in \Cref{fig:opt-buffered}). Then Ember adds a new variable in the SLC loop (\texttt{i}~variable in line 9 in \Cref{fig:opt-aligned}), replaces all uses of the stream-to-value operations with that variable (line 16--17 in \Cref{fig:opt-aligned}), and increments it in the end callback of its child loop (line 18 in \Cref{fig:opt-aligned}).
...
\Cref{fig:ember-vs-handopt} shows the performance of DAE code automatically generated and optimized by Ember (\texttt{emb-opt3}) compared to hand-optimized code (\texttt{ref-dae}) for all model classes in \Cref{sec:embedding-workload-characterization}. Besides all optimizations discussed in \Cref{sec:slc-level-optimizations}, hand-optimized code also includes low-level, CPU-specific optimizations to improve callback invocations. These CPU-specific optimizations include, for instance, (1)~reordering the if-cases of multi-callback code (e.g. \Cref{fig:opt-impact-aligned}) according to their taken frequency or (2)~setting the values of control tokens so they can be directly used in compute code (e.g. to increment variables).
