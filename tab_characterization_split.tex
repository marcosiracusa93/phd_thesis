\begin{table}[]
    \centering
    %\footnotesize
    \caption[Algorithmic characterization of embedding operations.]{Algorithmic characterization of the embedding operations studied in \Cref{sec:embedding-workload-characterization}. The table summarizes each operation's loop hierarchy and its compute-per-lookup ratio, defined as the number of vector compute operations performed per dense embedding-vector lookup. In general, these operations expose little arithmetic work per irregular lookup, limiting the ability of compute-oriented architectures to hide lookup latency.}
...
\begin{table}[]
    \centering
    %\footnotesize
    \caption[Memory characterization of embedding operations.]{Memory characterization of the embedding operations studied in \Cref{sec:embedding-workload-characterization}. The table reports embedding storage requirements, temporal locality measured by the reuse-distance CDF, and spatial locality measured by the number of elements in each embedding vector. Together, these properties determine how effectively caches, prefetchers, and coalescing mechanisms can serve embedding lookups.}
    %\resizebox{\columnwidth}{!}{%
    \begin{tabular}{ccccc}
        \toprule
            \makecell[c]{\textbf{Op}} & 
