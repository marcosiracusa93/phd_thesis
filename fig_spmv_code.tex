...
void spmv(row_ptrs: mref<? x idx>, // adj matrix CSR pointers
  ...
  for (idx row_idx=0; row_idx<n_rows; row_idx++){
    ...
    // Go through all non-zero in a row
    ...
    \end{lstlisting}
    \vspace{-1em}
    \caption[Pseudocode of sparse matrix-vector multiplication.]{Pseudocode of the SpMV in \Cref{fig:spmv-lookup}. The outer loop goes through all the matrix rows whereas the inner loop goes through all the non-zero element in each row. Line 14 looks up the dense vector, a critical operation in modern processors.}
    \label{fig:spmv-code}
\end{figure}