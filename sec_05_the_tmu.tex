...
We overcome the challenges discussed above by offloading critical tensor traversal and merging operations to a specialized unit, which we call the Tensor Marshaling Unit (TMU). This chapter introduces the TMU, its instruction set, its use in accelerating sparse tensor algebra operations, its microarchitecture, and a comprehensive evaluation of its performance and power efficiency.
...
Queues are sized with an analytical model that allocates space to layers according to the amount of data to load, which can be statically estimated from the number of nnzs per tensor fiber.
...
\Cref{fig:spmv_example_1} and \ref{fig:spmv_example_2} depict a step-by-step example for SpMV (\Cref{fig:spmv_decomposition}) where the TMU has been programmed using the code in \Cref{fig:spmv_programmability}. The example loads the CSR matrix shown in \Cref{fig:sparse_formats} and assumes a two-lane design.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we first evaluate TMU performance on scientific sparse tensor algebra kernels with respect to traditional architectures. Then, we provide a sensitivity analysis on TMU storage and SVE vector length. Finally, we compare the TMU to existing scalar engines. The next section evaluates the TMU for accelerating machine learning models.
...
For instance, as the TMU implements all the logic to traverse and load tensor operands using specialized dataflow hardware, it can issue memory requests with higher throughput and efficiency than the core. Moreover, because the TMU can run at a lower frequency, it can track 8$\times$ more outstanding requests without timing issues and with low power consumption (less than 2\% overhead, as in \Cref{sec:dae-potential-scientific}). \Cref{fig:bw-per-miss} demonstrates that the TMU can track 82\% of the outstanding memory requests required to achieve full throughput (one request per cycle), whereas the CPU can track only 16\%. Hence, as shown in \Cref{fig:ppw-scatter-plot} and \ref{fig:ppw-dae-improvement}, the TMU achieves 5.7$\times$ and 5.6$\times$ higher requests/s and requests/s/W than traditional cores, 5.2$\times$ and 6.3$\times$ better than doubling the core resources for outstanding requests (\Cref{fig:dse-cpu}). In this way, as shown in \Cref{fig:hbm-dae-improvement}, the TMU utilizes 4$\times$---8$\times$ more memory bandwidth than a traditional core. Hence, as shown in \Cref{fig:bw-scaling}, DAE architectures (1)~achieve higher performance with the same core count, and (2)~require fewer cores to achieve the same performance, achieving higher efficiency.
