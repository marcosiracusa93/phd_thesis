...
void eb(idxs: mref<? x idx>, offs: mref<? x idx>,
  ...
  slc.for(str s_b from 0 to num_batches){
    ...
    \label{fig:ember-slc-unoptimized-code}
    \end{minipage}
    ...
    \label{fig:ember-slc-vectorized-code}
    \end{minipage}
    ...
    \label{fig:ember-slc-buffered-code}
    \end{minipage}
    ...
    \label{fig:ember-slc-aligned-code}
    \end{minipage}
    
    \vspace{-0.8em}
    \caption[Progressive SLC optimization steps.]{Progressive optimization of the EB SLC IR. The figure shows the unoptimized SLC program, the vectorized SLCV program, the buffered version that materializes embedding vectors as compound stream values, and the queue-aligned version that regenerates simple indices on the execute side. Gray code is unchanged from the previous version. Overall, each step keeps the program structured while exposing a more efficient queue protocol.}
    \label{fig:ember-slc-optimization-transformations}
\end{figure}
