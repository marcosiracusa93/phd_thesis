...
% ---------- Table 3 ----------
\caption[TU traversal primitives.]{TU traversal primitives. For each signature, \texttt{int} arguments are constant values and \texttt{stream} arguments are the values loaded by the parent loop, as explained later. The \texttt{Dense} traversal implements a regular for-loop with static loop bounds. The \texttt{Range} traversal implements a for loop with data-dependent loop bounds, coming from two streams. The \texttt{Index} traversal implements a for loop that loads a dense structure with fixed size from arbitrary memory locations, whose starting location is indicated by the lower bound.}
\label{tab:traversal}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llll@{}}
\toprule
...
\texttt{ite} & generates TU iteration indexes $\in$[\texttt{beg},\texttt{end}) & \texttt{x:b->e}\\
